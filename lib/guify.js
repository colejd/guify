  ...
  function Interval(root, opts, theme) {
    ...
    _this.lValue.onchange = function () {
      ...
      if (Number(parseFloat(rawValue)) == rawValue) {
        ...
        value = Math.min(Math.max(value, _this.min), _this.max); // Map to nearest step

        if (_this.step) {
          value = Math.ceil((value - _this.min) / _this.step) * _this.step + _this.min;
